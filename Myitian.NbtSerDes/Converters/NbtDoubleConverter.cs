...
{
    public class NbtDoubleConverter : NbtConverterBase
    {
        public override byte ID => 6;

        public NbtDoubleConverter(NbtConverter converter) : base(converter) { }
        ...
        public override void Serialize(ref Stream stream, dynamic value)
        {
            ...
            {
                case byte i:
                    stream.Write(BitConv.GetBytes((double)i), 0, 4);
                    break;
                case sbyte i:
                    stream.Write(BitConv.GetBytes((double)i), 0, 4);
                    break;
        ...
        public override dynamic Deserialize(ref Stream stream, Type type)
        {
            ...
            if (type == typeof(double) || type == typeof(object))
            {
                ...
                if (read > 0)
                {
                    return BitConv.ToDouble(buffer, 0);
                }
            }
                ...
                type == typeof(float) || type == typeof(decimal))
            {
                ...
                if (read > 0)
                {
                    return Convert.ChangeType(BitConv.ToDouble(buffer, 0), type);
                }
            }
            else
            {
